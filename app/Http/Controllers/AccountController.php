...
Class AccountController extends Controller{
			...
			if (!$link){
				...
				return redirect('/update')->withInput()->withErrors('Connection could not be established.');
					...
					elseif(!$count_table){
						...
						if($request->has('ajax_submit')){
				            $response = ['message' => 'Updated successfully.', 'status' => 'success']; 
				            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
				        }
    ...
    public function postVerifyPurchase(Request $request){
    	...
    	if($data['status'] == 'status'){
    		if($request->has('ajax_submit')){
	            $response = ['message' => 'Your purchase is already verified.', 'status' => 'error']; 
	            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
	        }
    		return redirect('/');
    	}
		...
		if($data['status'] == 'success'){
			...
			complete($purchase_code);
			if($request->has('ajax_submit')){
	            $response = ['message' => $data['message'], 'status' => 'success']; 
	            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
	        }
   ...
   public function releaseLicense(){
	...
	public function store(AccountRequest $request){
        ...
        if(!preg_match('/^[a-zA-Z0-9_\.\-]*$/',$request->input('username'))){
            if($request->has('ajax_submit')){
                $response = ['message' => trans('messages.username_rules'), 'status' => 'error']; 
                return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
            }
            return redirect('/home')->withErrors(trans('messages.username_rules'));
        }
		...
		if (!is_writable('../config/db.php')){
			...
			if (!$link){
				...
				return redirect()->back()->withInput()->withErrors('Connection could not be established.');
					...
					elseif($count_table){
						if($request->has('ajax_submit')){
				            $response = ['message' => 'Table already exists. Installation needs empty database.', 'status' => 'error']; 
				            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
				        }
						return redirect()->back()->withInput()->withErrors('Table already exists. Installation needs empty database.');
							...
							write2Config($config,'code');
							if($request->has('ajax_submit')){
					            $response = ['message' => 'Installed successfully.', 'status' => 'success']; 
					            return response()->json($response, 200, array('Access-Controll-Allow-Origin' => '*'));
					        }
