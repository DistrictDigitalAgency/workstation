...
class WMLab
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        ...
        if(checkDBConnection()){
            ...
            foreach(config('template') as $key => $value){
                if(!\App\Template::where('slug',$key)->count()){
                    $insert[] = array('is_default' => 1,'name' => toWord($key),'category' => $value['category'],'slug' => $key);
                }
            }

            if(count($insert))
                \App\Template::insert($insert);
        }
        $menu = '';
        ...
        view()->share(compact('direction','default_localization','datatable_localization','calendar_localization','datepicker_localization','menus','menu','setup_guide','custom_field_values'));
